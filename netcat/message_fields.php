...
if ($posting) {
    ...
    do {
        ...
        for ($i = 0; $i < $fldCount; $i++) {
            ...
            $nc_field_value_is_inherited = false;
            if ($fldInheritance[$i]) {
                $nc_field_has_inherited_value = $nc_core->get_component($nc_core->get_system_table_name_by_id($SystemTableID), $fld[$i]) != '';
            }
            ...
            switch ($fldType[$i]) {
                ...
                case NC_FIELDTYPE_STRING:
                    ...
                    if ($fldNotNull[$i] && $fldValue[$i] == "" && !$nc_field_has_inherited_value && !($action == 'change' && $fld[$i] == $AUTHORIZE_BY)) {
                        $errCode = 1;
                    }
                    if ($fldNotNull[$i] && !$nc_field_has_inherited_value && $fldFmt[$i] == "url" && ($fldValue[$i] == 'http://' || $fldValue[$i] == 'ftp://')) {
                        $errCode = 1;
                    }
                ...
                case NC_FIELDTYPE_INT:
                    if ($fldNotNull[$i] && $fldValue[$i] == "" && !$nc_field_has_inherited_value) {
                        $errCode = 1;
                    }
                ...
                case NC_FIELDTYPE_FILE:
                    ...
                    if ($fldNotNull[$i] && ($fldValue[$i] == "" || $fldValue[$i] == "none") && !$nc_field_has_inherited_value) {
                        $errCode = 6;
                    }
                ...
                case NC_FIELDTYPE_MULTISELECT:
                    if ($fldNotNull[$i] && !count($fldValue[$i]) && !$nc_field_has_inherited_value) {
                        $errCode = 1;
                    }
                    $ClassificatorName = strtok($fldFmt[$i], ':');
                    $tmp = ",";
                ...
                case NC_FIELDTYPE_MULTIFILE:
                    ...
                    $not_null = $fldNotNull[$i] && count($_FILES["f_{$fld[$i]}_file"]['name']) == 0 && !$nc_field_has_inherited_value;
