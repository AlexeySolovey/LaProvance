...
if (!$perm->isInsideAdmin()) {
    ...
    echo "<div><a href='" . $SUB_FOLDER . $HTTP_ROOT_PATH . "modules/auth/?logoff=1&REQUESTED_FROM=/'>".BEGINHTML_LOGOUT."</a></div>";
    EndHtml ();
    exit;
}
...
function nc_subdivision_preview_link($data, $sid_off = false) {
    ...
    if ($data['ExternalURL']) {
        $result = strchr($data['ExternalURL'], ":")
                    ? $data['ExternalURL']
                    : "http://" .
                      ($data['Domain'] ? $data['Domain'] : $DOMAIN_NAME) .
                      $SUB_FOLDER .
        ...
        $result = "http://" .
                    ($data['Domain'] ? $data['Domain'] : $DOMAIN_NAME) .
                    nc_routing::get_folder_path($data['Subdivision_ID']);
    }
    else {
        $result = "http://" .
                    ($data['Domain'] ? $data['Domain'] : $DOMAIN_NAME) .
