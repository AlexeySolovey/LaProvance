...
function nc_file_field($field_name, $style = "", $classID = "", $caption = false) {
    ...
    if ($message && (($systemTableID && $value) || ($action == 'change' && ($old = $GLOBALS["f_" . $field_name . "_old"])))) {
        ...
        $file_size_string = nc_bytes2size($filesize);

        $result .= "<div class='nc-upload-file' data-type='" . htmlspecialchars($filetype, ENT_QUOTES) . "'>" .
                   "<div class='nc-upload-file-info'>" .
                   "<a class='nc-upload-file-name' href='$filepath'  target='_blank' title='" .
                   htmlspecialchars("$filename ($file_size_string)", ENT_QUOTES) . "'>" .
                   htmlspecialchars($filename) .
                   "</a> " .
                   "<span class='nc-upload-file-size'>$file_size_string</span> " .
                   "<a href='#' class='nc-upload-file-remove' title='" . NETCAT_MODERATION_FILES_DELETE . "'>×</a></div>" .
                   "<input id='k" . $field_id . "' type='hidden' class='nc-upload-file-remove-hidden' name='f_KILL" . $field_id . "' value='0' />" .
                   "</div>\r\n";
    }
